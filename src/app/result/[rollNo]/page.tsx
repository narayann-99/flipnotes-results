...
export default function ResultPage({ params }: PageProps) {
  ...
  const fetchData = useCallback(async (signal?: AbortSignal) => {
    ...
    try {
      ...
      if (err instanceof Error && err.name === "AbortError") {
        return; // Ignore component unmount aborts
      }
  ...
  // Error States (Result Not Found / Network Failure / Service Unavailable)
  if (error) {
    const is404 = errStatus === 404 || errCode === "NOT_FOUND";
    const isTimeout = errStatus === 408 || errCode === "TIMEOUT";

    let title = "Connection Interrupted";
    let message = "Unable to establish secure connection with university registry servers. Check your network connection.";

    if (is404) {
      title = "Transcript Not Found";
      message = "The requested roll number is not indexed in the registry database. Please verify and retry.";
    } else if (isTimeout) {
      title = "Registry Request Timed Out";
      message = "The university registry server took too long to respond. Please try again.";
    } else if (errCode === "INVALID_ROLL_NO") {
      title = "Invalid Roll Number";
      message = "The provided roll number format is invalid. Please check and re-enter.";
    } else if (errCode === "SERVICE_UNAVAILABLE") {
      title = "Registry Under Maintenance";
      message = error;
    }
