...
export class MockProvider implements ResultProvider {
  ...
  async getResult(rollNo: string, options?: ProviderOptions): Promise<ResultData> {
    ...
    const delayMs = options?.timeoutMs ? Math.min(2000, options.timeoutMs - 100) : 1500;
    await this.delay(delayMs, options?.signal);

    // Test triggers for simulating system failures in test environments
    if (cleanRollNo === "503503503503") {
      throw new ProviderError(
        "SERVICE_UNAVAILABLE",
        "University registry service is currently undergoing scheduled maintenance.",
        503,
        true
      );
    }

    if (cleanRollNo === "408408408408") {
      throw new ProviderError(
        "TIMEOUT",
        "Connection to registry server timed out after multiple attempts.",
        408,
        true
      );
    }

    // Standard student dataset (Primary Student: Om Narayan Mishra)
    if (cleanRollNo === "2504850100131") {
      return {
        student: {
    ...
    // Non-existent test roll number (all 0s or ending in 000)
    if (cleanRollNo === "0000000000000" || cleanRollNo.endsWith("000")) {
      throw new ProviderError(
        "NOT_FOUND",
        `No academic transcript record found for roll number ${cleanRollNo}.`,
        404,
        false
      );
    }

    // Dynamic generation for other valid 10-15 digit roll numbers
    ...
    const name = names[parseInt(cleanRollNo.slice(-3)) % names.length] || "Anshika Gupta";
    const sgpaList = [7.62, 8.04, 7.92, 8.41, 8.25, 8.56, 8.12, 8.35];
    const totalSems = (parseInt(cleanRollNo.slice(-1)) % 4) + 4;
    
    let sumSgpa = 0;
    const semesters: Semester[] = [];
    for (let i = 1; i <= totalSems; i++) {
      const baseSgpa = sgpaList[(parseInt(cleanRollNo.slice(-2)) + i) % sgpaList.length];
      ...
      if (i === totalSems && cleanRollNo.endsWith("9")) {
        subjects.push({
          code: `CS${i}04`,
  ...
  private delay(ms: number, signal?: AbortSignal): Promise<void> {
    return new Promise((resolve, reject) => {
      if (signal?.aborted) {
        return reject(new ProviderError("TIMEOUT", "Request aborted before completion.", 408, true));
      }
